...
## v4.0.4 — 16.04.2026 10:23

**Değişen dosya:** `index.html`

**Bug fix — 3 düzeltme:**
- **Artikel**: Kelime soyuldu — `"das Buch"` yerine `"Buch"` gösteriliyor (`word.replace(/^(der|die|das)\s+/)`)
- **Mini Sınav**: `SINAV_BOLUMLER` tanımsız hatası düzeltildi — A1/A2 için `getSinavA1A2()` fonksiyonu eklendi, GQ havuzundan mcq+fill sorularını sinav formatına dönüştürür
- **JSON Import**: `validateQuestion()` Gemini/GPT çıktılarına toleranslı hale getirildi: `a:"2"` string → int, metin cevap → indeks arama, `correct/answer/options/choices` alias alanları kabul

---

## v4.0.3 — 15.04.2026 05:41

**Değişen dosya:** `index.html`

**Düzeltilenler:**
- `Cannot set properties of null (setting 'className')` hatası düzeltildi
- `switchImportTab`: classList yerine className string replace kullanıldı (null-safe)
- `switchImportTab` ve `showPage` çağrıları `setTimeout` ile geciktirildi (DOM hazır olmadan çalışıyordu)
- `setGenOption`: null parent kontrolü eklendi
- `showPage("home")` için doğru nav butonu seçici düzeltildi
...
## v4.0.3 — 15.04.2026 05:50

**Değişen dosya:** `index.html`

**Bug fix — sadece düzeltmeler, yeni özellik yok:**
- Bozuk `<!-- comment<title>...</title>` tek satırı → ayrı comment + title satırına düzeltildi
- `switchImportTab`: `className` string replace + `\x08` regex → `classList.remove/add` ile yeniden yazıldı
- `showPage`: tüm element erişimleri null-safe yapıldı (`if (el)` kontrolü)
- Import tab init: `document.getElementById("tab-generate")` null kontrolü eklendi
- `window.load` → `showPage("home")` querySelector yerine güvenli onclick attr kontrolü
- Drive sync dokunulmadı

---

## v4.0.2 — 14.04.2026 13:15

**Değişen dosya:** `index.html`

**Düzeltilenler:**
- Ana sayfa yerine "Soru Ekle" panelinin açılması sorunu düzeltildi
- `ex-panel` varsayılan olarak `display:none` yapıldı (görünür kalıyordu)
- `window.load` sonunda `showPage("home")` çağrısı eklendi
- `DOMContentLoaded`'dan `updateSubcatButtons()` kaldırıldı (import sayfası görünür olmadan çalışıyordu)
- `switchImportTab` + `showPage('import')` açılınca subcat butonları initialize ediliyor
